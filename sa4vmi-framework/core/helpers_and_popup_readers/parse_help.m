...
function [file_content,help_found]=parse_help(function_name)
    try
    file_content = ''; % contenuto iniziale dell'info box (inizializzazione)
    if exist(fullfile('functions',strcat(function_name,'.txt')), 'file') == 2
        fid = fopen(fullfile('functions',strcat(function_name,'.txt'))); % leggimi il file di help
        help_found = 1;
    else % se non c'è la guida, leggimi il codice
        fid = fopen(fullfile('functions',function_name));
        help_found = 0;
    end
    tline = fgetl(fid);
    while ischar(tline)
