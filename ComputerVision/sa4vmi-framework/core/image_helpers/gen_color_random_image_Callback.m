...
function gen_color_random_image_Callback(hObject, eventdata, handles)
    try
        handles.input_image = zeros(600,600,3);
        for x=1:600
            for y=1:600
                handles.input_image(x,y,1) = uint8(rand()*255);
                handles.input_image(x,y,2) = uint8(rand()*255);
                handles.input_image(x,y,3) = uint8(rand()*255);
            end
        end
